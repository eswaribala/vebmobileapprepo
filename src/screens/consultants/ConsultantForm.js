...
const ACCENT = '#0277BD';
const DEPARTMENTS = ['Orthodontics', 'Periodontics', 'Endodontics', 'Oral Surgery', 'Prosthodontics', 'Pediatric Dentistry', 'General'];

export default function ConsultantFormScreen({ route, navigation }) {
  ...
  useEffect(() => {
    ...
    navigation.setOptions({ title: existing?.id ? 'Edit Consultant' : 'Add Consultant' });
  }, []);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const data = { ...form, role: 'Consultant', salary: parseFloat(form.salary) || 0, is_active: 1 };
      if (existing?.id) {
        await staffAPI.update(existing.id, data);
      } else {
        await staffAPI.create(data);
      }
      Alert.alert('Success', `Consultant ${existing?.id ? 'updated' : 'added'} successfully`, [
  ...
  return (
    <ScrollView style={styles.container} keyboardShouldPersistTaps="handled">
      <View style={styles.card}>
        ...
        <TextInput style={styles.input} value={form.name} onChangeText={v => sf('name', v)} placeholder="Consultant full name" placeholderTextColor={theme.colors.textLight} />
        ...
        <Text style={styles.label}>Consulting Since</Text>
        ...
        <Text style={styles.label}>Consulting Fee (₹)</Text>
      ...
      <TouchableOpacity style={[styles.saveBtn, saving && { opacity: 0.6 }]} onPress={handleSave} disabled={saving}>
        {saving ? <ActivityIndicator color="#fff" /> : (
          <>
            ...
            <Text style={styles.saveBtnText}>{existing?.id ? 'Update Consultant' : 'Add Consultant'}</Text>
          </>
        )}
      </TouchableOpacity>
